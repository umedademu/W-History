...
地図と物語で世界史の流れを学ぶ、クリックで進む紙芝居教材です。現在の版は **v0.030** です。
...
入口は `/` の教材一覧です。前後ボタン、左右キー、場面番号から移動できます。音声と自動の場面送りはありません。全ページでライト・ダークを切り替えられ、選んだ表示は次回も引き継がれます。07は大分類やプルダウンを設けず、53個のページ番号から直接移動できます。一時停止や本文の名前からの見直しもでき、端末の「動きを減らす」設定では結果を静止表示します。
