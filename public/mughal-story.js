import { places, zones, scenes } from "./mughal-scenes.js?v=0.005";
...
const resolveImg = key => key.includes("/") ? `/images/${key}.png` : `/images/mughal/${key}.png`;
...
function drawMap(scene) {
  ...
  const base = svg("image", { href: "/timur-map.svg", x, y, width: 1260 * scale, height: 720 * scale });
  map.append(base);

  const regions = svg("g", { class: "mughal-regions" });
  map.append(regions);
  const polygon = points => points.map(p => toScreen(p).join(",")).join(" ");
  for (const key of scene.zones) {
    const z = zones[key];
    if (z) regions.append(svg("polygon", { points: polygon(z.points), fill: z.color, stroke: z.color, "stroke-width": 1.1, opacity: 0.35 }));
  }

  for (const [p, text] of [[[72, 14], "アラビア海"], [[86, 15], "ベンガル湾"], [[79, 8], "インド洋"]]) {
  ...
  const arrows = svg("g", { class: "mughal-arrows" });
  map.append(arrows);
  ...
  const cityLayer = svg("g", { class: "mughal-cities" });
