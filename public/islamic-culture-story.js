import { places, zones, scenes } from "./islamic-culture-scenes.js?v=0.005";
...
const resolveImg = key => key.includes("/") ? `/images/${key}.png` : `/images/islamic-culture/${key}.png`;
...
function drawMap(scene) {
  ...
  const regions = svg("g", { class: "culture-regions" });
  map.append(regions);
  const polygon = points => points.map(p => toScreen(p).join(",")).join(" ");
  for (const key of scene.zones) {
    const z = zones[key];
    if (z) regions.append(svg("polygon", { points: polygon(z.points), fill: z.color, stroke: z.color, "stroke-width": 1.1, opacity: 0.25 }));
  }
  ...
  const arrows = svg("g", { class: "culture-arrows" });
  map.append(arrows);
  ...
  const cityLayer = svg("g", { class: "culture-cities" });
