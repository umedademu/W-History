import {series,volumeNavigation} from './story-volumes.js?v=0.081';
import {mountStory} from './history-story.js?v=0.081';
const id=location.pathname.split('/').pop().replace(/-story\.html$/,'');
const volume=series.find(v=>v.id===id);
const {chapterEdition,chapterPlaces}=await import('./chapter-'+String(volume.chapter).padStart(2,'0')+'-edition.js?v=0.081');
