...
const expectedTitles=['イスラーム教の成立〜正統カリフ時代','ウマイヤ朝とアッバース朝','3カリフの並立とイスラーム世界の変容','地方政権の興亡','ティムール朝','サファヴィー朝','オスマン帝国','ムガル帝国','イスラーム文化'];
assert.equal(series.length,9);
assert.deepEqual(series.map(s=>s.label),expectedTitles);
assert.deepEqual(series.map(s=>s.number),Array.from({length:9},(_,i)=>String(i+1).padStart(2,'0')));
assert.deepEqual(series.map(s=>[s.lesson,s.part]),[[20,1],[20,2],[20,3],[20,4],[21,1],[21,2],[21,3],[21,4],[21,5]]);
assert.deepEqual(series.map(s=>volumeScenes(sourceEdition,s.id).length),[22,19,16,19,9,9,27,20,14]);
// 原文照合用14区分の全155場面が、同じ順番で一度ずつ掲載される。
...
if(!process.argv.includes('--published')){
 ...
 assert.deepEqual([...chapter.matchAll(/^\* \d+ (.*?) ……/gm)].map(m=>m[1]),expectedTitles);
}
assert.equal([...catalog.matchAll(/class="story-card"/g)].length,9);
for(const [lesson,count] of [[20,4],[21,5]]){
...
for(const s of series){
 ...
 assert.equal([...nav.matchAll(/<a /g)].length,9);
...
console.log('目次と同じ全9パート（第20回4・第21回5）、155場面の順序・欠落・重複なし、旧入口の転送先を確認しました。');
