...
import {chapterEdition,chapterPlaces} from '../public/chapter-04-edition.js';
import {chapterSeries} from '../public/chapter-04-volumes.js';
import {chapterNamesInText} from '../public/chapter-04-geography.js';
...
const reference=JSON.parse(read('docs/chapter-04/source-selection.json'));
const plan=JSON.parse(read('docs/chapter-04/reading-plan.json'));
const generatedPlan=JSON.parse(read('docs/chapter-04/page-plan.json'));
const pages=Object.values(chapterEdition).flat();
const decode=s=>s.replace(/<[^>]*>/g,'').replaceAll('&quot;','"').replaceAll('&gt;','>').replaceAll('&lt;','<').replaceAll('&amp;','&');
assert.equal(chapterSeries.length,18);assert.equal(pages.length,141);assert.equal(reference.paragraphs.length,300);
...
for(const [index,scene] of pages.entries()){
 assert.equal(scene.sourceText.chapter,4);assert.equal(scene.title,plan[index].title);assert.deepEqual(scene.sourceText.passages,plan[index].passages);
...
assert.ok(chapterNamesInText('ウマイヤ朝').some(n=>n.name==='ウマイヤ朝'));
assert.ok(chapterNamesInText('プラハ大学').some(n=>n.name==='プラハ大学'));
assert.ok(!chapterNamesInText('神の愛と騎士道精神').length);
const review=JSON.parse(read('docs/chapter-04/name-review.json'));
for(const name of review.required)assert.ok(chapterNamesInText(name).some(n=>n.key===normalizeMapName(name)),'確認済み固有名詞 '+name);
execFileSync(process.execPath,[fileURLToPath(new URL('scripts/build-chapter-04.mjs',root)),'--check'],{stdio:'pipe'});
console.log('第4章: 全'+pages.length+'ページの原文・太字・順序・地図・再生成を確認しました。');
