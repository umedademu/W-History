...
import {chapterSeries,chapterLessons} from '../public/chapter-03-volumes.js';
import {chapterEdition,chapterPlaces} from '../public/chapter-03-edition.js';
import {chapterNamesInText} from '../public/chapter-03-geography.js';
import {sceneMapItems} from '../public/map-name-coverage.js';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..');
const read=p=>fs.readFileSync(path.join(root,p),'utf8');
const {files,paragraphs}=JSON.parse(read('docs/chapter-03/source-selection.json'));
const reading=JSON.parse(read('docs/chapter-03/reading-plan.json'));
const reviewed=JSON.parse(read('docs/chapter-03/bold-name-review.json'));
const published=process.argv.includes('--published');
const decode=s=>s.replace(/<[^>]*>/g,'').replaceAll('&quot;','"').replaceAll('&gt;','>').replaceAll('&lt;','<').replaceAll('&amp;','&');
assert.equal(chapterSeries.length,16);
assert.deepEqual(chapterLessons.map(l=>l.lesson),[8,9,10]);
assert.equal(reading.length,107);
assert.equal(paragraphs.length,168);
...
for(const v of chapterSeries){assert.equal(v.chapter,3);assert.match(v.id,/^c03-l\d{2}-p\d{2}$/);assert.deepEqual(v.sections,[v.id]);
 for(const [i,s] of chapterEdition[v.id].entries()){
  const p=reading[pageIndex++];assert.equal(p.volume,v.id);assert.equal(s.title,p.title);assert.equal(s.id,v.id+'-'+String(i+1).padStart(3,'0'));assert.equal(s.sourceText.chapter,3);assert.deepEqual(s.sourceText.passages,p.passages);assert.equal(s.body.length,p.passages.length);assert.equal(s.plainBody.length,p.passages.length);
...
assert.deepEqual(chapterNamesInText('司馬炎（武帝）').find(e=>e.name==='武帝').points[0],[112.45,34.62]);
assert.ok(paragraphs.find(p=>p.id==='c03-8-87').text.includes('悪いやツ'));
assert.ok(chapterEdition['c03-l08-p02'].some(s=>s.plainBody.join('').includes('じゃあ、封建制ってどんな制度')&&s.plainBody.join('').includes('封土を与えるかわり')));
for(const name of ['高宗','靺鞨人','安南都護府','北庭都護府','大斉','武宗','孔子','鳩摩羅什'])assert.ok(chapterNamesInText(name).some(e=>e.name===name));
execFileSync(process.execPath,['scripts/build-chapter-03.mjs','--check'],{cwd:root,stdio:'inherit'});
console.log('第3章：全文・太字・掲載順・全行分類・地図名・自然な接続・再生成一致を確認（'+reading.length+'ページ）');
