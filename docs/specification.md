...
現在の版は v0.006（2026年9月6日）。中東史全9編、合計153場面、場面で使う画像171点を収録しています。参考書全33枚の情報を体系化した資料集（sourcesフォルダ・全5編＋対照表）を整理し、保管しています（公開管理からは除外）。
...
`npm run check` は構文、ローカル参照先、教材の入口、Ankiへの依存、公開設定を検査します。追加５編は地名・座標・経路・人物・章の入口・操作部品を全場面で検査します。実際の表示は `npm run dev` で確認します。`npm run build` も同じ検査を実施し、生成処理なしで `public` を公開します。
