...
現在の版は v0.029（2026年9月7日）。中東史全9編、合計227場面、場面で使う画像168点を収録しています。参考書全33枚の情報を完全に書き起こした資料集（sourcesフォルダ・全5編＋対照表）を整理し、保管しています（公開管理からは除外）。
...
`npm run check` は構文、ローカル参照先、教材の入口、Ankiへの依存、公開設定を検査します。追加５編は地名・座標・経路・人物・章の入口・操作部品を全場面で検査します。01は27場面・原資料由来197項目、02は27場面・原資料由来249項目について、同じ場面の本文と地図への双方向掲載、話題の順序、地図に出す地点名・地域名の本文掲載、対象範囲外の先取り語を検査します。07は地図の向き・表示範囲・位置案内・移動の切り替えに加え、53ページ・90段落で本文と地図の名前の双方向一致、原資料の名前165項目、28主題の説明順と重要な経緯、地図の位置・経路・効果・画像の対応も検査します。08・09は原資料に沿う場面順と、流れを支える重要語の保持を検査します。実際の表示は `npm run dev` で確認します。`npm run build` も同じ検査を実施し、生成処理なしで `public` を公開します。
